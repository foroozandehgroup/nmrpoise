...
#      I have used os.path over pathlib, but if we switch to pathlib, then this is >=3.4 at least.
...
def main():
    ...
    with open(os.path.join(p_costfunctions, routine.cf), "rb") as f:
        cf = dill.load(f)
    # (possible) TODO: May want to check that the Routine and Cost_Function objects are valid

    # Scale the initial values and tolerances
    x0 = scale(routine.init, routine.lb, routine.ub)
    xtol = np.array(routine.tol) / (np.array(routine.ub) - np.array(routine.lb))
    xatol_gm = np.prod(xtol)**(1/np.size(xtol))
    # TODO: Scipy's optimisers don't allow for an xatol vector -- there is no way to differentiate
    #       between different tolerances for different parameters. For now I have opted to use
    #       the geometric mean of the (scaled) xtol vector as the input to xatol. In order to change
    #       this behaviour we would need to change the actual optimiser, i.e. move away from scipy.
    ...
    with open(p_optlog, "a") as log:
        ...
        print("\n\n", file=log)
        print(now, file=log)
        print("Starting optimisation of the following parameters: {}".format(routine.pars), file=log)
    ...
    opt_result = optimize.minimize(acquire_nmr, x0, args=[cf, routine.lb, routine.ub, p_spectrum, p_optlog], method="Nelder-Mead", options={'xatol': xatol_gm, 'fatol': np.inf, 'disp': False})
...
def acquire_nmr(x, optimargs):
    ...
    Sends a signal to the frontend script to run an NMR acquisition using the scaled parameter values contained in x.
    Then, waits for the spectrum to be acquired and processed; and then calculates the cost function associated with the spectrum.
    If any of the values in x are out-of-bounds, this simply returns np.inf as the cost function.

    Arguments:
        x (ndarray of floats): values to be used in evaluation of cost function, scaled to be between 0 and 1.
        optimargs (tuple)    : contains various parameters needed for other tasks, like communication with frontend.
...
def scale(val, lb, ub):
    """
    Scales a set of values to between 0 and 1 (which are the lower and upper optimisation bounds respectively). Returns None if any of the values are outside the bounds.
...
def ppm_to_point(shift):
    """
    Round a specific chemical shift to the nearest point in the spectrum. Needs the global variable p_spectrum to be set.

    Arguments:
        shift (float) : desired chemical shift.

    Returns:
        (int): the desired point. None if the chemical shift lies outside the spectral window.
    """
    si = get_proc_par("SI")
...
def get_fid():
    """
    Returns the FID as a (complex) np.ndarray. Needs the global variable p_spectrum to be set.
    Note that this does *not* deal with the "group delay" at the beginning of the FID.
    """
...
def get_real_spectrum(left=None, right=None):
    ...
    Note that the effects of TopSpin's NC_PROC variable are removed by this function.

    Arguments:
        (optional) left (float) : chemical shift corresponding to beginning of the desired secttion
        (optional) right (float): chemical shift corresponding to end of the desired secttion
        If the argument left (or right) is not passed, then the array returned will stretch to the left (or right) end of the spectrum.
    ...
    if left == None and right == None: # bounds not specified
        return real_spec

    # Swap both bounds if they were both specified, but not in the correct order
...
def get_acqu_par(par):
    """
    Obtains the value of an acquisition parameter. Needs the global variable p_spectrum to be set.

    Arguments:
        par (str): Name of the acquisition parameter.

    Returns:
        (float) value of the acquisition parameter. None if the value is not a number, or if the parameter doesn't exist.
    """
    ...
    parr = par[len(parl):]
    params_with_space = ["CNST", "D", "P", "PLW", "PCPD", "GPX", "GPY", "GPZ", "SPW", "SPOAL", "SPOFFS", "L", "IN", "INP", "PHCOR"]

    # Get the parameter
    if (parr != "") and (parl in params_with_space): # e.g. cnst2
        with open(p_acqus, "r") as file:
            # Read up to the line declaring the parameters; the values will start on the subsequent line
            for line in file:
                if line.upper().startswith("##${}=".format(parl)):
                    break
            # Grab the values (by reading until the next parameter begins) and put them in a list
            s = ""
...
def get_proc_par(par):
    """
    Obtains the value of a processing parameter. Needs the global variable p_spectrum to be set.

    Arguments:
        par (str): Name of the processing parameter.

    Returns:
        (float) value of the processing parameter. None if the value is not a number, or if the parameter doesn't exist.
    """
    # Construct path to acqus file
    ...
    # Get the value (for processing parameters there aren't any lists like CNST/D/P)
    with open(p_acqus, "r") as file:
        for line in file:
            if line.upper().startswith("##${}=".format(par)):
                try:
...
def getpar(par):
    """
    Obtains the value of an (acquisition or processing) parameter. Tries to search for an acquisition parameter first, then processing.

    Arguments:
        par (str): Name of the parameter.

    Returns:
        (float) value of the parameter. None if the value is not a number, or if the parameter doesn't exist.
    """
